...
### This scripts add thumbnails to each file ###
thumbnail="$(echo $2 | sed 's/.mp3$//').png"
...
for i in "${!OUT[@]}"; do
  ffmpeg -ss "${SSI[i]}" -to "${TO2[i]}" -i "$2" -i "$thumbnail" -map 0 -map 1 -c copy -c:a:1 -disposition:a:1 "${OUT[i]}.mp3"
done
