...
then
  ...
  #grep -Fvxf /tmp/file1 $HOME/Downloads/dirs-db > /tmp/cleaned.txt
  awk 'NR==FNR{a[$0];next} !($0 in a)' /tmp/file1 $HOME/Downloads/dirs-db > /tmp/cleaned.txt
  cat /tmp/file2 >> /tmp/cleaned.txt
  cat /tmp/cleaned.txt > $HOME/Downloads/dirs-db
  LC_ALL=C sort -u -o $HOME/Downloads/dirs-db $HOME/Downloads/dirs-db
fi
rm -vf /tmp/file* /tmp/cleaned.txt &>/dev/null
